...
configure_dnscrypt() {
    ...
    if ! wget -q -O "${CONFIG_FILE}.tmp" "$PRECONFIGURED_CONFIG_URL"; then
        ...
        # Проверка и исправление несовместимых параметров
        log "INFO" "Проверка совместимости конфигурации с текущей версией DNSCrypt-proxy"
        
        # Проверка версии DNSCrypt-proxy
        local dnscrypt_version=$(dnscrypt-proxy -version 2>/dev/null | head -n 1 | awk '{print $2}' || echo "unknown")
        log "INFO" "Обнаруженная версия DNSCrypt-proxy: $dnscrypt_version"
        
        # Получаем информацию о версии пакета
        if dpkg -l | grep -q dnscrypt-proxy; then
            local pkg_version=$(dpkg -l | grep dnscrypt-proxy | awk '{print $3}')
            log "INFO" "Версия установленного пакета: $pkg_version"
        fi
        
        # Список известных несовместимых параметров для разных версий
        local known_issues=(
            "odoh_servers"       # Проблема с поддержкой Oblivious DoH в старых версиях
            "monitoring_ui"      # Мониторинг UI может быть недоступен в некоторых версиях
            "http3_probe"        # HTTP3 параметры в старых версиях
            "cloak_ptr"          # Параметр для cloaking PTR записей
        )
        
        # Проверка и комментирование несовместимых параметров
        log "INFO" "Адаптация конфигурации для текущей версии..."
        for issue in "${known_issues[@]}"; do
            if grep -q "^${issue}\s*=" "${CONFIG_FILE}.tmp" || grep -q "^\[${issue}\]" "${CONFIG_FILE}.tmp"; then
                log "WARN" "Найден потенциально несовместимый параметр: $issue, комментируем"
        ...
        if grep -q -E "^\[odoh_servers\]|^\[odoh-servers\]" "${CONFIG_FILE}.tmp"; then
            log "WARN" "Найдена несовместимая секция [odoh_servers], комментируем всю секцию"
            sed -i '/^\[odoh_servers\]/,/^\[/s/^/#/' "${CONFIG_FILE}.tmp"
        fi
        
        if grep -q -E "^\[monitoring_ui\]" "${CONFIG_FILE}.tmp"; then
            log "WARN" "Найдена потенциально несовместимая секция [monitoring_ui], комментируем всю секцию"
    ...
    # Проверяем реальное имя пользователя dnscrypt-proxy в системе
    local actual_dnscrypt_user=""
    if getent passwd "_dnscrypt-proxy" >/dev/null; then
        actual_dnscrypt_user="_dnscrypt-proxy"
    elif getent passwd "dnscrypt-proxy" >/dev/null; then
        actual_dnscrypt_user="dnscrypt-proxy"
    elif getent passwd "_dnscrypt" >/dev/null; then
        actual_dnscrypt_user="_dnscrypt"
    elif getent passwd "dnscrypt" >/dev/null; then
        actual_dnscrypt_user="dnscrypt"
    fi
    
    if [ -n "$actual_dnscrypt_user" ]; then
        log "INFO" "Установка владельца $actual_dnscrypt_user для конфигурационных файлов"
        chown -R "${actual_dnscrypt_user}:${actual_dnscrypt_user}" "$CONFIG_DIR" || {
            log "WARN" "Не удалось изменить владельца $CONFIG_DIR на $actual_dnscrypt_user"
            ...
            find "$CONFIG_DIR" -exec chown "${actual_dnscrypt_user}:${actual_dnscrypt_user}" {} \; 2>/dev/null || {
                log "WARN" "Не удалось установить владельца файлов. Продолжаем с текущими правами."
            }
        }
    else
        log "WARN" "Пользователь DNSCrypt-proxy не найден в системе"
        log "INFO" "Установка общедоступных прав чтения для конфигурационных файлов"
        chmod -R a+r "$CONFIG_DIR"
    fi
    
    # Проверяем конфигурацию на наличие синтаксических ошибок
    log "INFO" "Проверка конфигурации на наличие ошибок"
    cd "$(dirname "$CONFIG_FILE")" && dnscrypt-proxy -check -config="$CONFIG_FILE" && \
        log "SUCCESS" "Конфигурация успешно проверена" || {
        log "ERROR" "Обнаружены ошибки в конфигурации. Попытка автоматического исправления..."
        
        # Резервное копирование проблемной конфигурации
        cp "$CONFIG_FILE" "${CONFIG_FILE}.error"
        
        # Загрузка стандартного примера и наложение базовых настроек
        log "INFO" "Использование стандартной конфигурации с минимальными настройками"
        if wget -q -O "${CONFIG_FILE}.fix" "$EXAMPLE_CONFIG_URL"; then
            sed -i "s/^listen_addresses = .*/listen_addresses = ['127.0.0.1:53']/" "${CONFIG_FILE}.fix"
            sed -i "s/^server_names = .*/server_names = ['adguard-dns', 'quad9-dnscrypt-ip4-filter-ecs-pri']/" "${CONFIG_FILE}.fix"
            mv "${CONFIG_FILE}.fix" "$CONFIG_FILE"
            
            # Повторная проверка
            cd "$(dirname "$CONFIG_FILE")" && dnscrypt-proxy -check -config="$CONFIG_FILE" && \
                log "SUCCESS" "Базовая конфигурация успешно применена" || {
                log "ERROR" "Проблемы с конфигурацией не устранены. Установка может завершиться некорректно."
                return 1
            }
        else
            log "ERROR" "Не удалось загрузить стандартную конфигурацию для исправления ошибок"
            return 1
        fi
    }
