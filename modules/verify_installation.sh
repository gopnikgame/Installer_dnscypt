...
verify_installation() {
    ...
    if [ -f "$DNSCRYPT_CONFIG" ]; then
        ...
        if [ $? -eq 0 ] && [ -n "$dnscrypt_bin" ]; then
            ...
            if cd "$(dirname "$DNSCRYPT_CONFIG")" && "$dnscrypt_bin" -check -config="$DNSCRYPT_CONFIG" 2>/dev/null; then
                ...
                safe_echo "${YELLOW}Попытка диагностики проблем с конфигурацией:${NC}"
    ...
    if command -v lsof &>/dev/null; then
        ...
        if [ -n "$dnscrypt_ports" ]; then
            safe_echo "${GREEN}DNSCrypt прослушивает следующие порты:${NC}"
    ...
    if [ -r "$DNSCRYPT_CONFIG" ]; then
        ...
        safe_echo "${RED}Нет доступа для чтения${NC}"
        ((errors++))
    fi
    ...
    if [ -f "/etc/dnscrypt-proxy/public-resolvers.md" ]; then
        safe_echo "${GREEN}Найдены${NC}"
    else
        safe_echo "${YELLOW}Отсутствуют${NC}"
    ...
    if [ $errors -eq 0 ]; then
        ...
        safe_echo "${GREEN}✓ Служба работает и прослушивает порты${NC}"
        safe_echo "${GREEN}✓ DNS резолвинг функционирует${NC}"
        safe_echo "${GREEN}✓ Конфигурация валидна${NC}"
        return 0
        ...
        safe_echo "${YELLOW}  DNSCrypt в основном работает корректно${NC}"
        return 0
    else
        print_header "РЕЗУЛЬТАТЫ ПРОВЕРКИ"
